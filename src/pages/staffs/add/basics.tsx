...
const AddBasicProfile = ({ isEdit = false }: { isEdit?: boolean }) => {
  ...
  const fetchLoading = useSelector((state) => state.staff.fetchLoading);
  ...
  useEffect(() => {
    if (singleStaff && id) {
      form.reset({
        firstName:
          singleStaff?.data?.attributes?.user?.data?.attributes?.firstName ||
          "",
        lastName:
          singleStaff?.data?.attributes?.user?.data?.attributes?.lastName || "",
        email:
          singleStaff?.data?.attributes?.user?.data?.attributes?.email || "",
        phone:
          singleStaff?.data?.attributes?.user?.data?.attributes?.phone || "",
        dob:
          new Date(
            singleStaff?.data?.attributes?.user?.data?.attributes?.dob
          ) || "",
        occupation:
          singleStaff?.data?.attributes?.user?.data?.attributes?.occupation ||
          "",
        gender:
        ...
        avatar: (() => {
          const dataTransfer = new DataTransfer();
          if (
            singleStaff?.data?.attributes?.user?.data?.attributes?.avatar?.url
          ) {
            const file = new File([], singleStaff.avatar.url);
              ...
              <Card className="my-6 p-2 sm:p-6 flex-[2]">
                <CardHeader>
                  <CardTitle className="flex items-center gap-20">
                    {isEdit
                      ? `Update Basic Info of ${singleStaff?.data?.attributes?.user?.data?.attributes?.username}`
