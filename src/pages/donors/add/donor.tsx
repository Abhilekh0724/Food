...
import { updateDonorProfile } from "@/store/features/donor-slice";
...
const AddDonorInfo = ({ isEdit = false }: { isEdit?: boolean }) => {
  ...
  const fetchLoading = useSelector((state) => state.donor.fetchLoading);
  ...
  const onSubmit = (data: FormValues) => {
    ...
    if (data?.attachments && data?.attachments?.length > 0) {
      for (let i = 0; i < data?.attachments?.length; i++) {
        devLog(data?.attachments?.[i], "file", i)
        formData.append("files.attachments", data?.attachments?.[i], data?.attachments?.[i]?.name);
      }
    }
    ...
    const questionaries = {
      ...
      last_donated_date: data.lastDonationDate,
    };

    delete (data as Partial<FormValues>).hadVaccinationLast3Month;
    delete (data as Partial<FormValues>).receiveDirectCall;
    delete (data as Partial<FormValues>).receiveDirectMessage;
    delete (data as Partial<FormValues>).everReceivedBlood;
    delete (data as Partial<FormValues>).donatedBloodBefore;
    delete (data as Partial<FormValues>).victimOfAnyDisease;

    devLog(data, "datataatatatatatatta")
    appendFormData(formData, {
      data: JSON.stringify({
        ...data,
        questionaries,
        lastDonationDate: data.lastDonationDate.toISOString(),
      })
    });
    ...
    dispatch(
      updateDonorProfile({
        data: formData,
        navigate,
        id: singleDonor?.attributes?.donor?.data?.attributes?.donorProfile?.data
          ?.id,
        actionBy: user?.id,
      })
    );
  ...
  useEffect(() => {
    if (singleDonor && id) {
      form.reset({
        height:
          singleDonor?.attributes?.donor?.data?.attributes?.donorProfile?.data
            ?.attributes?.height || "",
        weight:
          singleDonor?.attributes?.donor?.data?.attributes?.donorProfile?.data
            ?.attributes?.weight || "",
        lastDonationDate: singleDonor?.attributes?.donor?.data?.attributes
          ?.donorProfile?.data?.attributes?.lastDonationDate
          ? new Date(
            singleDonor?.attributes?.donor?.data?.attributes?.donorProfile?.data?.attributes.lastDonationDate
          )
          : new Date(),
        everReceivedBlood:
          singleDonor?.attributes?.donor?.data?.attributes?.donorProfile?.data
            ?.attributes?.questionares?.everReceivedBlood || false,
        donatedBloodBefore:
          singleDonor?.attributes?.donor?.data?.attributes?.donorProfile?.data
            ?.attributes?.questionares?.donatedBloodBefore || false,
        victimOfAnyDisease:
          singleDonor?.attributes?.donor?.data?.attributes?.donorProfile?.data
            ?.attributes?.questionares?.victimOfAnyDisease || false,
        hadVaccinationLast3Month:
          singleDonor?.attributes?.donor?.data?.attributes?.donorProfile?.data
            ?.attributes?.questionares?.hadVaccinationLast3Month || false,
        receiveDirectCall:
          singleDonor?.attributes?.donor?.data?.attributes?.donorProfile?.data
            ?.attributes?.questionares?.receiveDirectCall || false,
        receiveDirectMessage:
          singleDonor?.attributes?.donor?.data?.attributes?.donorProfile?.data
            ?.attributes?.questionares?.receiveDirectMessage || false,
      });
    }
  }, [singleDonor, form, id]);
              ...
              <Card className="my-6 p-2 sm:p-6 flex-[2]">
                <CardHeader>
                  <CardTitle className="flex items-center gap-20">
                    {isEdit
                      ? `Update Donor Info of ${singleDonor?.attributes?.donor?.data?.attributes?.username}`
                      : "Add Donor Info of Donor"}
                  </CardTitle>
                </CardHeader>
